...
const handleChargeFromEnergyTab = async (group: any) => {
    ...
    try {
        ...
        // --- Upload do Recibo (PDF Unificado) ---
        try {
            // Nota: Assume que createUnifiedPDFDoc e uploadPaymentDocument estão disponíveis no escopo
            const { doc, tenantName } = await (window as any).createUnifiedPDFDoc(group);
            const pdfBase64 = doc.output('datauristring');

            await (window as any).uploadPaymentDocument(
                payment.id,
                pdfBase64,
